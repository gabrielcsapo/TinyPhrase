...
void round(bool isRandomMode) {
  while(gameState == IN_A_ROUND) {
    ...
    int totalGameOptions = selectedMenuOption.length;
    char* currentGameItem = selectedMenuOption.options[lastGameOption];

    printCenteredText(currentGameItem);
    ...
    for(int i = 0; i <= 5; i++){
      ...
      if(ret!=0){
        if(micros()-lastPress>DEBOUNCE_BUTTON){
          if(ret == BUTTON_RIGHT) {
            if(isRandomMode) {
              // we want to ignore the all of the above game option
              int totalMenuOptions = sizeof(MenuOptions) / sizeof(MenuOptions[0]) - 1;

              // let's selected a random menu and set the game options so that we can continue the program flow without any differences.
              selectedMenuOption = MenuOptions[random(0, totalMenuOptions)];
              totalGameOptions = selectedMenuOption.length;
            }

            lastGameOption = random(0, totalGameOptions);
          }
    ...
    case MAIN_MENU: {
      ...
      switch(getButton()) {
        case BUTTON_RIGHT: {
          ...
          if(selectedMenuOption.menuItemName == "All of the Above") {
            isRandomGameMode = true;
          }

          int totalMenuOptions = sizeof(MenuOptions) / sizeof(MenuOptions[0]) - 1;

          // let's selected a random menu and set the game options so that we can continue the program flow without any differences.
          lastMenuOption = random(0, totalMenuOptions);
        } break;
        case BUTTON_LEFT:
